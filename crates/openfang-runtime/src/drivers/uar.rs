...
//!
//! Wraps UAR's [`LiterLlmDriver`] to give OpenFang agents access to 142+
...
use universal_agent_runtime::{
    config::{LlmConfig, build_client_config},
...
impl LlmDriver for UarDriver {
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        let mut tool_calls: Vec<openfang_types::tool::ToolCall> = Vec::new();
        ...
        while let Some(event) = stream.next().await {
            match event.map_err(|e| LlmError::Http(e.to_string()))? {
                ...
                NormalizedEvent::ToolCallComplete {
                    ...
                    tool_calls.push(openfang_types::tool::ToolCall { id, name, input });
                }
            ...
            match &event {
                ...
                NormalizedEvent::ToolCallDelta {
                    ...
                    if let Some(delta) = arguments_delta {
                        let _ = tx
                            .send(StreamEvent::ToolInputDelta { text: delta.clone() })
                            .await;
                    }
                }
                NormalizedEvent::ToolCallComplete {
                    ...
                    let _ = tx
                        ...
                        .await;
                    tool_calls.push(openfang_types::tool::ToolCall {
                        id: id.clone(),
                        name: name.clone(),
                        input,
                    });
...
/// Convert an OpenFang [`CompletionRequest`] into a UAR [`LlmRequest`].
///
            ...
            (Role::Assistant, MessageContent::Blocks(blocks)) => {
                ...
                for block in blocks {
                    match block {
                        ...
                        // Thinking blocks are not forwarded — providers that support
                        // thinking don't need the prior round's trace.
                        ContentBlock::Thinking { .. }
                        | ContentBlock::ToolResult { .. }
                        | ContentBlock::Image { .. }
                        | ContentBlock::Unknown => {}
